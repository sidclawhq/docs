...
By default, the supervisor only learns about subagent completion when it uses the check tool. For a more proactive experience, add custom [middleware](/oss/javascript/deepagents/customization#middleware) to the subagent graph that sends a message to the supervisor's thread when the subagent finishes.
