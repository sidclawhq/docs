...
By default, the supervisor only learns about subagent completion when it uses the check tool. For a more proactive experience, add `CompletionNotifierMiddleware` to the subagent's [middleware](/oss/python/deepagents/customization#middleware) stack. When the subagent finishes (or errors), the middleware sends a message to the supervisor's thread, waking the supervisor so it can proactively relay results.
